...
`linear_mcp.ts` and `macro_mcp.ts` use the reusable helper in `mcp_bridge/bridge.ts` to start remote MCP servers. On Linux it uses the Nix/Home Manager `mcp-remote` command; on macOS and other platforms it uses `npx -y mcp-remote`.
